...
export async function subscribeEmail(page, email) {
    try {
        console.log(`Starting subscription for ${email} to AP News`);
        ...
        for (const selector of checkboxSelectors) {
            try {
                ...
                console.log(`Clicked checkbox ${selector} for ${email}`);
            } catch (error) {
                console.warn(`Error clicking checkbox ${selector} for ${email}: ${error.message}`);
                continue;
            }
        }
        ...
        for (let i = 0; i < 3; i++) {
            try {
                ...
                await page.waitForTimeout(1000);
                console.log(`Clicked SELECT button (first) - click ${i+1} for ${email}`);
            } catch (error) {
                console.warn(`Error clicking SELECT button (first) - click ${i+1} for ${email}: ${error.message}`);
            }
        }
        ...
        try {
            ...
            await page.waitForTimeout(1000);
            console.log(`Clicked Enable accessibility for ${email}`);
        } catch (error) {
            console.warn(`Error clicking Enable accessibility for ${email}: ${error.message}`);
        }
        ...
        try {
            ...
            await selectButton.click();
            await page.waitForTimeout(1000);
            console.log(`Clicked SELECT button (first) again for ${email}`);
        } catch (error) {
            console.warn(`Error clicking SELECT button (first) again for ${email}: ${error.message}`);
        }
        ...
        try {
            ...
            await selectButton.click();
            await page.waitForTimeout(1000);
            console.log(`Clicked SELECT button (no first) for ${email}`);
        } catch (error) {
            console.warn(`Error clicking SELECT button (no first) for ${email}: ${error.message}`);
        }
        ...
        try {
            ...
            await page.waitForTimeout(1000);
            console.log(`Filled email address field with clean email: ${cleanEmail}`);
        } catch (error) {
        ...
        try {
            ...
            await page.waitForTimeout(2000);
            console.log(`Clicked disclaimer checkbox for ${email}`);
        } catch (error) {
            console.warn(`Error clicking disclaimer checkbox for ${email}: ${error.message}`);
        }
        ...
        try {
            ...
            console.error(`Error clicking submit button for ${email}: ${error.message}`);
            throw error;
        }
        
        await page.waitForTimeout(5000);
        console.log(`Subscribed ${email} to AP News newsletters`);
    } catch (error) {
        console.error(`Error subscribing ${email} to AP News: ${error.message}`);
        throw error;
    }
}
