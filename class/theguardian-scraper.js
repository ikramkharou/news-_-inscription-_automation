...
export class TheGuardianScraper extends BaseScraper {
    getUrl() {
    ...
    async subscribeEmail(page, email) {
        try {
            logger.info(`Starting subscription for ${email} to The Guardian`);
            ...
            for (const buttonName of newsletterButtons) {
                try {
                    ...
                    logger.warn(`Error clicking newsletter button '${buttonName}' for ${email}: ${error.message}`);
                    continue;
                }
            }
            ...
            try {
                ...
                await page.waitForTimeout(3000);
                logger.info(`Toggled updates checkbox for ${email}`);
            } catch (error) {
                logger.warn(`Error toggling updates checkbox for ${email}: ${error.message}`);
            }
            ...
            try {
                ...
                if (frame) {
                    ...
                    await page.waitForTimeout(3000);
                    logger.info(`Clicked reCAPTCHA checkbox for ${email}`);
                }
            } catch (error) {
                logger.warn(`Error clicking reCAPTCHA checkbox for ${email}: ${error.message}`);
            }
            ...
            try {
                ...
                await page.waitForTimeout(3000);
                logger.info(`Clicked final div element for ${email}`);
            } catch (error) {
                logger.warn(`Error clicking final div element for ${email}: ${error.message}`);
            }
            
            await page.waitForTimeout(5000);
            logger.info(`Subscribed ${email} to The Guardian newsletters`);
        } catch (error) {
            logger.error(`Error subscribing ${email} to The Guardian: ${error.message}`);
            throw error;
        }
    }
}
