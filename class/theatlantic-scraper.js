...
export async function subscribeEmail(page, email) {
    try {
        console.log(`Starting subscription for ${email} to The Atlantic`);
        ...
        for (const text of newsletterClicks) {
            try {
                ...
                try {
                    ...
                    console.warn(`Exact match timed out for '${text}', trying partial match for ${email}`);
                ...
                try {
                    ...
                    if (clickError.message.includes("intercepts") || clickError.message.toLowerCase().includes("intercepted")) {
                        console.warn(`Click intercepted for '${text}', trying force click for ${email}`);
                ...
                console.warn(`Error clicking newsletter item '${text}' for ${email}: ${error.message}`);
                continue;
            }
        }
        ...
        try {
            for (let i = 0; i < 4; i++) {
                ...
                console.log(`Clicked Select button ${i+1} (first) for ${email}`);
            }
            ...
            await selectButton.click();
            await page.waitForTimeout(3000);
            console.log(`Clicked Select button 5 (no first) for ${email}`);
        } catch (error) {
            console.warn(`Error clicking Select buttons for ${email}: ${error.message}`);
        }
        ...
        console.log(`Subscribed ${email} to The Atlantic newsletters`);
    } catch (error) {
        console.error(`Error subscribing ${email} to The Atlantic: ${error.message}`);
        throw error;
    }
}
